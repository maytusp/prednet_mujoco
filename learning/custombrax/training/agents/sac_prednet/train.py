...
"""Soft Actor-Critic training with PredNet auxiliary losses.
...
from custombrax.training import acting
from custombrax.training import gradients
from custombrax.training import pmap
from custombrax.training import replay_buffers
from custombrax.training import types
from custombrax.training.acme import running_statistics
from custombrax.training.acme import specs
...
def _make_preprocess_observations_fn(
    normalize_observations: bool, task_dim: int
):
  ...
  def normalize_state_only(
    ...
    state_obs = observation[..., :-task_dim]
    task = observation[..., -task_dim:]
...
def _init_training_state(
  ...
  task_params = jax.random.normal(key_task, (task_dim,), dtype=jnp.float32)
...
def train(
  ...
  task_dim = sf_dim if prednet_use_task_vector else 0
  if prednet_use_task_vector and sf_dim <= 0:
    raise ValueError('sf_dim must be positive when prednet_use_task_vector=True')

  policy_obs_size = obs_size + task_dim if task_dim else obs_size
  normalize_fn = _make_preprocess_observations_fn(
      normalize_observations, task_dim
  )
  ...
  def sgd_step(
    ...
    metrics.update(
        {f'prednet/{name}': value for name, value in critic_metrics.items()}
    )
    if task_dim:
      metrics.update(
          {f'prednet/{name}': value for name, value in task_metrics.items()}
      )
  ...
  def get_experience(
    ...
    policy = make_policy(
        _make_policy_params(
            normalizer_params, policy_params, task_params, task_dim
        )
    )
  ...
  if restore_checkpoint_path is not None:
    ...
    if task_dim and len(params) == 3:
      ...
      training_state = training_state.replace(
          ...
          task_params=(
              params[4] if len(params) > 4 else training_state.task_params
          ),
      )

  if restore_params is not None:
    restore_params = _replicate_tree(restore_params, local_devices_to_use)
    if task_dim and len(restore_params) == 3:
      training_state = training_state.replace(
  ...
  policy_params = (params[0], params[1], params[4]) if task_dim else params[:2]
  ...
  return (make_policy, params if return_q_params else policy_params, metrics)
